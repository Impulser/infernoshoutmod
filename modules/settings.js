define(['./mod', 'text!./settings/settings.html', './settings/frontend'], function (InfernoShoutMod, settingsHtml, settingsJs) {
	...
	/* Usage:
        mod.addCustomSetting('some-setting',
            'Some Label',
            $('<input id="somesetting-label" type="checkbox" checked="checked">'),
                function (e) {
                    alert($('#somesetting-label').prop("checked"));
                })
        );
	*/

	// Note context can be a jQuery object or plain HTML.

	InfernoShoutMod.prototype.addCustomSetting = function (id, label, context, callback) {
		this.customSettings || (this.customSettings = {});

		var $setting = $('<label for="' + id + '">' + label + '</label><div id="' + id + '" class="placeholder"> </div>');
		$setting.appendTo('#infernoshoutmod-customsettings');
		$('#' + id + '.placeholder')[context instanceof jQuery ? 'append' : 'html'](context);
